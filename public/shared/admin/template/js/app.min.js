...
function getUsers(search = "", online = "") {
    $.ajax({
        url: site_adr + "post.php?admin=getUser",
        type: "POST",
...
function notify(message, type) {
    if (type == "danger") var title = msg_error + '!';
    if (type == "success") var title = msg_success + '!';
    if (type == "info") var title = msg_info + '!';
    else var title = "Неизвестно!";
...
function save() {
    ...
    $.ajax({
        url: site_adr + "post.php?admin=save",
        type: "POST",
        data: {
            "save": save
        },
...
function statePost(id, state) {
    $.ajax({
        url: site_adr + "post.php?admin=state_post",
        type: "POST",
...
function editor(id, h) {
    $(id).summernote({
        ...
        toolbar: [
            ...
            ['height', ['height']],
            ['table', ['table']],
            ['insert', ['link', 'picture', 'video', 'hr', 'hello']],
...
function addPost() {
    $.ajax({
        url: site_adr + "post.php?admin=add_post",
        type: "POST",
        data: {
            ...
            'short': $('div.short_post').summernote('code'),
            'full': $('div.full_post').summernote('code')
        },
        success: function (data) {
            informer(data);
            //alert(data);
...
function deletePost(id) {
    $.get(site_adr + "post.php?admin=delete_post&id=" + id, function (data) {
...
function updatePost(id) {
    $.ajax({
        url: site_adr + "post.php?admin=update_post",
        type: "POST",
        data: {
            'id': id,
            'title': $('input[name=title]').val(),
...
function deleteRow(r) {
    ...
    document.getElementById("table_routing").deleteRow(i);
    notify("Маршрут удален", "danger");
}
$.fn.serializeArrayWC = function (type) {
    if (type == "bool") var msg = {yes: true, no: false}; else var msg = {yes: "yes", no: "no"};
    var rCRLF = /\r?\n/g;
        ...
        .map(function (i, elem) {
            if (this.type == "checkbox") {
                if (this.checked == true) {
                    return {name: this.name, value: msg.yes};
                }
                if (this.checked == false) {
                    return {name: this.name, value: msg.no};
                }
            }
...
$(document).ready(function () {
    ...
    editor('div.short_post', 300);
    editor('div.full_post', 500);
    var route = {};

    $("button[name=save_routing]").on('click', function () {
        $.each($('input[name^="route\\["]').serializeArrayWC(), function () {
            var saver = this.name.replace('route[', '');
            route[saver] = this.value;
        });
        $.ajax({
            url: site_adr + "post.php?admin=save_routing",
            type: "POST",
            data: {"route": route},
            success: function (data) {
                informer(data);
    ...
    $("button[name=add_routing]").on('click', function () {
        var i = randomInteger(50, 9999);
        $('tbody').prepend("<tr bgcolor='#15423b'><td><input class='form-control' title='' type='text' name='route[" + i + "-0]' value=''/></td> <td><input class='form-control' title='' type='text' name='route[" + i + "-1]' value=''/></td> <td><input class='form-control' title='' type='text' name='route[" + i + "-2]' value=''/></td> <td><div class='toggle-switch'><input class='toggle-switch__checkbox' title='' type='checkbox' name='route[" + i + "-3]' /><i class='toggle-switch__helper'></i></div></td><td><button class='btn btn--light btn--icon' onclick='deleteRow(this)'><i style='color: red;' class='zmdi zmdi-close'></i></button></td></tr>");
